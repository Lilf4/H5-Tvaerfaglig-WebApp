...
// @ts-nocheck
    ...
    async function acceptRequest(){
        await Post("process_request", {"request_id": request_id, "accepted": true, "reason": noteText}, {"session-token": GetSessionToken()})
        goto("/admin-page/request-overview")
    }

    async function declineRequest(){
        await Post("process_request", {"request_id": request_id, "accepted": false, "reason": noteText}, {"session-token": GetSessionToken()})
        goto("/admin-page/request-overview")
    }
</script>
<style>
...
{#if ready}
    <BackButton backPage="/admin-page/request-overview"/>
    <div id="Content">
            <h1>Process Request</h1>
            <label id="RequestedByLabel" for="RequestedByInput">
                Requested By
                <input id="RequestedByInput" type="text" bind:value={User.name} disabled>
            </label>
            ...
            <label id="NoteTextLabel" for="NoteTextInput">
                Note
                <textarea id="NoteTextInput" bind:value={noteText} placeholder="Enter note..."></textarea>
            </label>
            <div class="SpacerDiv"></div>
            <div id="ButtonDiv">
                <button type="button" on:click={async () => {acceptRequest()}}>Accept</button>
                <button type="button" on:click={async () => {declineRequest()}}>Decline</button>
            </div>
